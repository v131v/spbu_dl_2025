"""
Year Prediction MSD - Improved Solution
Увеличенная модель для лучшего качества

Улучшения:
- Большая модель: [512, 384, 256, 128, 64] вместо [256, 128, 64]
- Больше эпох: 200 вместо 100
- Больше patience: 25 вместо 15
...
print('='*60)
print('Year Prediction MSD - Improved Solution')
print('='*60)
print(f'Device: {device}')
print('Larger model + longer training for better quality')
print('Expected training time: 1-2 minutes on CPU')
print('='*60)


class ImprovedYearPredictionNet(nn.Module):
    """
    Улучшенная нейронная сеть с большим количеством параметров
    Архитектура: 90 → 512 → 384 → 256 → 128 → 64 → 1
    """
    def __init__(self, input_dim=90, hidden_dims=[512, 384, 256, 128, 64], dropout_rate=0.3):
        super(ImprovedYearPredictionNet, self).__init__()
        ...
        self.residual_proj1 = nn.Linear(hidden_dims[0], hidden_dims[2])  # 512 -> 256
        self.residual_proj2 = nn.Linear(hidden_dims[2], hidden_dims[4])  # 256 -> 64
    ...
    def forward(self, x):
        ...
        # Первые два скрытых блока
        out = self.hidden_blocks[0](out)  # 512 -> 384
        out = self.hidden_blocks[1](out)  # 384 -> 256
        ...
        # Следующие блоки
        out = self.hidden_blocks[2](out)  # 256 -> 128
        out = self.hidden_blocks[3](out)  # 128 -> 64
...
def main():
    ...
    # Улучшенная модель
    model = ImprovedYearPredictionNet(
        input_dim=90,
    ...
    print(f"Architecture: 90 → 512 → 384 → 256 → 128 → 64 → 1")
    ...
    train_losses, val_losses = train_model(
        ...
        num_epochs=200,  # Увеличено с 100
        device=device,
